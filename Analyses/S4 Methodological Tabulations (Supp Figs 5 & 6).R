...
pdf("Figures/Supplementary/Supp Figure 7 - Global Region Methodologies/Supp Figure 7 - Global Region Methodologies.pdf", width = 12.5, height = 8)
par(mfrow = c(2, 2))
...
legend(1, 1.1, c("Nested", "PCR-LDR", "qPCR", "RT-PCR", "Semi-Nested"), fill = palette(), ncol = 3, title = "PCR Methodology")
...
Normalised_LM_Fields_By_Continent <- rbind("Asia & Oceania" = table(Asia_Oceania$Microscopy_Fields)/sum(table(Asia_Oceania$Microscopy_Fields)), "East Africa" = table(East_Africa$Microscopy_Fields)/sum(table(East_Africa$Microscopy_Fields)),
                                                 "South America" = table(South_America$Microscopy_Fields)/sum(table(South_America$Microscopy_Fields)), "West Africa" = table(West_Africa$Microscopy_Fields)/sum(table(West_Africa$Microscopy_Fields)))
palette(c("#F15025", "#F2328C", "#7ACC70", "#00A7E1", "#8C00FF"))
...
Normalised_LM_Leucs_By_Continent <- rbind("Asia & Oceania" = table(Asia_Oceania$Microscopy_Leucocytes)/sum(table(Asia_Oceania$Microscopy_Leucocytes)), "East Africa" = table(East_Africa$Microscopy_Leucocytes)/sum(table(East_Africa$Microscopy_Leucocytes)),
                                           "South America" = table(South_America$Microscopy_Leucocytes)/sum(table(South_America$Microscopy_Leucocytes)), "West Africa" = table(West_Africa$Microscopy_Leucocytes)/sum(table(West_Africa$Microscopy_Leucocytes)))
...
sub_data_frame <- sub_data_frame[1:229, ] # removes NAs which are surveys not conducted in Africa for which Trans_Hist data was not available 
...
pdf("Figures/Supplementary/Supp Figure 8 - Trans Arch Methodologies/Supp Figure 8 - Transmission Archetype Methodologies.pdf", width = 12.5, height = 8)
par(mfrow = c(2, 2))
...
Normalised_LM_Fields_By_Archetype <- rbind("High_High" = table(high_high_subset$Microscopy_Fields)/sum(table(high_high_subset$Microscopy_Fields)), 
                                           "High Low" = table(high_low_subset$Microscopy_Fields)/sum(table(high_low_subset$Microscopy_Fields)), 
                                           "Low Low" = table(low_low_subset$Microscopy_Fields)/sum(table(low_low_subset$Microscopy_Fields)))
palette(c("#F15025", "#F2328C", "#7ACC70", "#00A7E1", "brown"))
barplot(t(Normalised_LM_Fields_By_Archetype), beside = TRUE, ylim = c(0, 1.1), las = 1, ylab = "Proportion of Studies", col = palette())
legend(1, 1, c("100", "200", "300", "400", "1000"), fill = palette(), ncol = 4, title = "Number of Fields Checked")
...
Normalised_LM_Leucs_By_Archetype <- rbind("High_High" = table(high_high_subset$Microscopy_Leucocytes)/sum(table(high_high_subset$Microscopy_Leucocytes)), 
                                          "High Low" = table(high_low_subset$Microscopy_Leucocytes)/sum(table(high_low_subset$Microscopy_Leucocytes)), 
                                          "Low Low" = table(low_low_subset$Microscopy_Leucocytes)/sum(table(low_low_subset$Microscopy_Leucocytes)))
palette(c("#F15025", "#F2328C", "#F9E931", "brown", "#7ACC70", "#00A7E1", "#8C00FF"))
