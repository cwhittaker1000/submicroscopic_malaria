...
old_micr_PCR_comp_model <- jags.model('1_Reversed_Basic_Model_Old_And_New.txt',   # OLD DATA
                                      data = old_microscopy_PCR_comparison, n.chains = 1, inits = inits)

new_micr_PCR_comp_model <- jags.model('1_Reversed_Basic_Model_Old_And_New.txt',   # NEW DATA
                                      data = new_microscopy_PCR_comparison, n.chains = 1, inits = inits)

full_micr_PCR_comp_model <- jags.model('1_Reversed_Basic_Model_Old_And_New.txt',   # ALL DATA
...
new_fitted_PCR <- expit(new_fitted_logit_PCR)
full_fitted_PCR <- expit(full_fitted_logit_PCR)
...
lines(old_fitted_PCR, Micro_prevalence, col = "blue", lwd = 3)
lines(new_fitted_PCR, Micro_prevalence, col = "orange", lwd = 3)
lines(full_fitted_PCR, Micro_prevalence, col = "black", lwd = 3)
...
lines(old_fitted_PCR, Micro_prevalence, col = "black", lwd = 3)
...
plot(old_data$PCR_Prev, old_data$Micro_Prev/old_data$PCR_Prev, xlim = c(0, 1), ylim = c(0, 1), pch = 20, col = "black",
     xlab = "PCR Prevalence", ylab = "Slide Prevalence")
lines(old_fitted_PCR, Micro_prevalence/old_fitted_PCR, col = "black", lwd = 3)
...
plot(new_data$PCR_Prev, new_data$Micro_Prev, xlim = c(0, 1), ylim = c(0, 1), pch = 20, col = "black",
     xlab = "PCR Prevalence", ylab = "Slide Prevalence")
lines(new_fitted_PCR, Micro_prevalence, col = "black", lwd = 3)
lines(seq(0,1,0.01), seq(0,1,0.01), col = "black", lwd = 1)
