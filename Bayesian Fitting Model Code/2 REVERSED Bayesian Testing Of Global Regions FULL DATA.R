...
Asia <- data_frame[data_frame$Global_Region == "Asia" | data_frame$Global_Region == "Oceania" & (data_frame$Full_Or_Age_Disagg_Data == 1 | data_frame$Full_Or_Age_Disagg_Data == 2), ]
sum(Asia$Micro_Prev == 0) #6 zeroes

East_Africa <- data_frame[data_frame$Global_Region == "East Africa" & (data_frame$Full_Or_Age_Disagg_Data == 1 | data_frame$Full_Or_Age_Disagg_Data == 2), ]
sum(East_Africa$Micro_Prev == 0) #6 zeroes

South_America <- data_frame[data_frame$Global_Region == "South America" & (data_frame$Full_Or_Age_Disagg_Data == 1 | data_frame$Full_Or_Age_Disagg_Data == 2), ]
sum(South_America$Micro_Prev == 0) #12 zeroes

West_Africa <- data_frame[data_frame$Global_Region == "West Africa" & (data_frame$Full_Or_Age_Disagg_Data == 1 | data_frame$Full_Or_Age_Disagg_Data == 2), ]
sum(West_Africa$Micro_Prev == 0) #2 zeroes

# When removing the studies with zero values NO LONGER NECESSARY
Asia <- data_frame[(data_frame$Global_Region == "Asia" | data_frame$Global_Region == "Oceania") & (data_frame$Full_Or_Age_Disagg_Data == 1 | data_frame$Full_Or_Age_Disagg_Data == 2) & data_frame$Was_Initially_Zero. == "N", ]
East_Africa <- data_frame[data_frame$Global_Region == "East Africa" & (data_frame$Full_Or_Age_Disagg_Data == 1 | data_frame$Full_Or_Age_Disagg_Data == 2) & data_frame$Was_Initially_Zero. == "N", ]
South_America <- data_frame[data_frame$Global_Region == "South America" & (data_frame$Full_Or_Age_Disagg_Data == 1 | data_frame$Full_Or_Age_Disagg_Data == 2) & data_frame$Was_Initially_Zero. == "N", ]
West_Africa <- data_frame[data_frame$Global_Region == "West Africa" & (data_frame$Full_Or_Age_Disagg_Data == 1 | data_frame$Full_Or_Age_Disagg_Data == 2) & data_frame$Was_Initially_Zero. == "N", ]
...
Asia_microscopy_PCR_comparison <- list(prev_pcr = Asia$PCR_N_Positive, ## number positive by PCR,
                                       ...
                                       N = 66)               # Total sample data overall
                                        #75 with zeroes, 66 without zeroes
...
East_Africa_microscopy_PCR_comparison <- list(prev_pcr = East_Africa$PCR_N_Positive, ## number positive by PCR,
                                              ...
                                              total_microscopy = East_Africa$Microscopy_N_Tested, ## number tested by microscopy,
                                                N = 154)            # Total sample data overall
# 154 zeroes not included, 161 zeroes included  
...
West_Africa_microscopy_PCR_comparison <- list(prev_pcr = West_Africa$PCR_N_Positive, ## number positive by PCR,
                                              ...
                                              N = 75) 
# 75 zeroes not included, 77 zeroes included  
...
Asia_micr_PCR_comp_model <- jags.model('1_Reversed_Basic_Model_Old_And_New.txt',  
                                       data = Asia_microscopy_PCR_comparison, n.chains = 1, inits = inits)

East_Africa_micr_PCR_comp_model <- jags.model('1_Reversed_Basic_Model_Old_And_New.txt',   
                                              data = East_Africa_microscopy_PCR_comparison, n.chains = 1, inits = inits)

South_America_micr_PCR_comp_model <- jags.model('1_Reversed_Basic_Model_Old_And_New.txt',   
                                                data = South_America_microscopy_PCR_comparison, n.chains = 1, inits = inits)

West_Africa_micr_PCR_comp_model <- jags.model('1_Reversed_Basic_Model_Old_And_New.txt',   
                                              data = West_Africa_microscopy_PCR_comparison, n.chains = 1, inits = inits)

# Oceania_micr_PCR_comp_model <- jags.model('1_Reversed_Basic_Model_Old_And_New.txt',   
...
Asia_micr_PCR_comp_model <- coda.samples(Asia_micr_PCR_comp_model, params, 
                                         n.iter = 10000, thin = 70) # Model updating
...
Micro_prevalence_Asia <- seq(0,0.85,0.001)
Asia_logit_Micro_prevalence <- logit(Micro_prevalence_Asia)

Micro_prevalence_East_Africa <- seq(0,0.95,0.001)
East_Africa_logit_Micro_prevalence <- logit(Micro_prevalence_East_Africa)

Micro_prevalence_West_Africa <- seq(0,1,0.001)
West_Africa_logit_Micro_prevalence <- logit(Micro_prevalence_West_Africa)

Micro_prevalence_South_America <- seq(0,0.5,0.001)
South_America_logit_Micro_prevalence <- logit(Micro_prevalence_South_America)

# PCR_prevalence_Oceania <- seq(0,0.8,0.001)
# Oceania_logit_Micro_prevalence <- logit(PCR_prevalence_Oceania)
...
Asia_fitted_logit_PCR <- Asia_delt_mean + (1 + Asia_beta_mean)  * Asia_logit_Micro_prevalence
East_Africa_fitted_logit_PCR <- East_Africa_delt_mean + (1 + East_Africa_beta_mean) * East_Africa_logit_Micro_prevalence
South_America_fitted_logit_PCR <- South_America_delt_mean + (1 + South_America_beta_mean)  * South_America_logit_Micro_prevalence
West_Africa_fitted_logit_PCR <- West_Africa_delt_mean + (1 + West_Africa_beta_mean) * West_Africa_logit_Micro_prevalence
# Oceania_fitted_logit_PCR <- Oceania_delt_mean + (1 + Oceania_beta_mean) * Oceania_logit_Micro_prevalence

plot(logit(Asia$PCR_Prev), logit(Asia$Micro_Prev))
lines(Asia_fitted_logit_PCR, Asia_logit_Micro_prevalence)

plot(logit(East_Africa$Micro_Prev), logit(East_Africa$PCR_Prev))
lines(East_Africa_logit_Micro_prevalence, East_Africa_fitted_logit_PCR)

plot(logit(West_Africa$PCR_Prev), logit(West_Africa$Micro_Prev))
lines(West_Africa_fitted_logit_PCR, West_Africa_logit_Micro_prevalence)

plot(logit(South_America$Micro_Prev), logit(South_America$PCR_Prev))
lines(South_America_logit_Micro_prevalence, South_America_fitted_logit_PCR)

# Converting them to the natural scale
Asia_fitted_PCR <- expit(Asia_fitted_logit_PCR)
East_Africa_fitted_PCR <- expit(East_Africa_fitted_logit_PCR)
West_Africa_fitted_PCR <- expit(West_Africa_fitted_logit_PCR)
South_America_fitted_PCR <- expit(South_America_fitted_logit_PCR)
# Oceania_fitted_PCR <- expit(Oceania_fitted_logit_PCR)

# 95% credible interval plotting

# Asia
Asia_values <- seq(0, 0.85, 0.001)
...
points(East_Africa$PCR_Prev, East_Africa$Micro_Prev, xlim = c(0, 1), ylim = c(0, 1), pch = 20, col = "green3")
...
lines(Asia_fitted_PCR, Asia_values, col = "black", lwd = 3)
polygon(y = c(Asia_values, rev(Asia_values)), 
        x = c(Asia_credible_upper, rev(Asia_credible_lower)), 
...
lines(East_Africa_fitted_PCR, East_Africa_values, col = "green3", lwd = 3)
polygon(y = c(East_Africa_values, rev(East_Africa_values)), 
        x = c(East_Africa_credible_upper, rev(East_Africa_credible_lower)), 
...
lines(South_America_fitted_PCR, South_America_values, col = "blue", lwd = 3)
polygon(y = c(South_America_values, rev(South_America_values)), 
        x = c(South_America_credible_upper, rev(South_America_credible_lower)), 
...
lines(West_Africa_fitted_PCR, West_Africa_values, col = "red", lwd = 3)
polygon(y = c(West_Africa_values, rev(West_Africa_values)), 
        x = c(West_Africa_credible_upper, rev(West_Africa_credible_lower)), 
...
lines(Oceania_fitted_PCR, Oceania_values, col = "turquoise", lwd = 3)
polygon(y = c(Oceania_values, rev(Oceania_values)), 
        x = c(Oceania_credible_upper, rev(Oceania_credible_lower)), 
...
plot(Asia$PCR_Prev, Asia$Micro_Prev/Asia$PCR_Prev, xlim = c(0, 1), ylim = c(0, 1), pch = 20, col = "black",
     xlab = "PCR Prevalence", ylab = "Slide Prevalence")
lines(Asia_fitted_PCR, Asia_values/Asia_fitted_PCR, col = "black", lwd = 3)

Asia_data_values <- seq(0,0.85,0.001)
Asia_data_chains_sens <- Asia_micr_PCR_comp_model[[1]]
Asia_data_pred_mean_dist_sens <- matrix(NA, nrow = nrow(Asia_data_chains_sens), ncol = length(Asia_data_values))
for (i in 1:nrow(Asia_data_pred_mean_dist_sens)){
  Asia_data_value_logit_scale <- Asia_data_chains_sens[i, "delt"] + 
    (1 + Asia_data_chains_sens[i, "beta"]) * logit(Asia_data_values)
  Asia_data_pred_mean_dist_sens[i, ] <- expit(Asia_data_value_logit_scale)
}
Asia_data_credible_lower_sens <- apply(Asia_data_pred_mean_dist_sens, MARGIN = 2, quantile, prob = 0.025)
Asia_data_credible_upper_sens <- apply(Asia_data_pred_mean_dist_sens, MARGIN = 2, quantile, prob = 0.975)

# Actually = lower because doing the 1 -
Asia_data_sensitivity_upper <- Asia_data_credible_upper_sens / Asia_data_values

# Actually = upper because doing the 1 -
Asia_data_sensitivity_lower <- Asia_data_credible_lower_sens / Asia_data_values

polygon(x = c(Asia_data_values, rev(Asia_data_values)), 
        y = c(Asia_data_sensitivity_upper, rev(Asia_data_sensitivity_lower)), 
        col = adjustcolor("black", alpha.f = 0.5), border = NA)

### East Africa

plot(East_Africa$PCR_Prev, East_Africa$Micro_Prev/East_Africa$PCR_Prev, xlim = c(0, 1), ylim = c(0, 1), pch = 20, col = "green3",
     xlab = "PCR Prevalence", ylab = "Slide Prevalence")
lines(East_Africa_fitted_PCR, East_Africa_values/East_Africa_fitted_PCR, col = "green3", lwd = 3)

East_Africa_data_values <- seq(0,0.95,0.001)
East_Africa_data_chains_sens <- East_Africa_micr_PCR_comp_model[[1]]
East_Africa_data_pred_mean_dist_sens <- matrix(NA, nrow = nrow(East_Africa_data_chains_sens), ncol = length(East_Africa_data_values))
for (i in 1:nrow(East_Africa_data_pred_mean_dist_sens)){
  East_Africa_data_value_logit_scale <- East_Africa_data_chains_sens[i, "delt"] + 
    (1 + East_Africa_data_chains_sens[i, "beta"]) * logit(East_Africa_data_values)
  East_Africa_data_pred_mean_dist_sens[i, ] <- expit(East_Africa_data_value_logit_scale)
}
East_Africa_data_credible_lower_sens <- apply(East_Africa_data_pred_mean_dist_sens, MARGIN = 2, quantile, prob = 0.025)
East_Africa_data_credible_upper_sens <- apply(East_Africa_data_pred_mean_dist_sens, MARGIN = 2, quantile, prob = 0.975)

# Actually = lower because doing the 1 -
East_Africa_data_sensitivity_upper <- East_Africa_data_credible_upper_sens / East_Africa_data_values

# Actually = upper because doing the 1 -
East_Africa_data_sensitivity_lower <- East_Africa_data_credible_lower_sens / East_Africa_data_values

polygon(x = c(East_Africa_data_values, rev(East_Africa_data_values)), 
        y = c(East_Africa_data_sensitivity_upper, rev(East_Africa_data_sensitivity_lower)), 
        col = adjustcolor("green3", alpha.f = 0.5), border = NA)
...
plot(South_America$PCR_Prev, South_America$Micro_Prev/South_America$PCR_Prev, xlim = c(0, 1), ylim = c(0, 1), pch = 20, col = "blue",
     xlab = "PCR Prevalence", ylab = "Slide Prevalence")
lines(South_America_fitted_PCR, South_America_values/South_America_fitted_PCR, col = "blue", lwd = 3)

South_America_data_values <- seq(0,0.5,0.001)
South_America_data_chains_sens <- South_America_micr_PCR_comp_model[[1]]
South_America_data_pred_mean_dist_sens <- matrix(NA, nrow = nrow(South_America_data_chains_sens), ncol = length(South_America_data_values))
for (i in 1:nrow(South_America_data_pred_mean_dist_sens)){
  South_America_data_value_logit_scale <- South_America_data_chains_sens[i, "delt"] + 
    (1 + South_America_data_chains_sens[i, "beta"]) * logit(South_America_data_values)
  South_America_data_pred_mean_dist_sens[i, ] <- expit(South_America_data_value_logit_scale)
}
South_America_data_credible_lower_sens <- apply(South_America_data_pred_mean_dist_sens, MARGIN = 2, quantile, prob = 0.025)
South_America_data_credible_upper_sens <- apply(South_America_data_pred_mean_dist_sens, MARGIN = 2, quantile, prob = 0.975)

# Actually = lower because doing the 1 -
South_America_data_sensitivity_upper <- South_America_data_values /  South_America_data_credible_upper_sens

# Actually = upper because doing the 1 -
South_America_data_sensitivity_lower <- South_America_data_values / South_America_data_credible_lower_sens 

polygon(x = c(South_America_data_values, rev(South_America_data_values)), 
        y = c(South_America_data_sensitivity_lower, rev(South_America_data_sensitivity_upper)), 
        col = adjustcolor("blue", alpha.f = 0.5), border = NA)
...
plot(West_Africa$PCR_Prev, West_Africa$Micro_Prev/West_Africa$PCR_Prev, xlim = c(0, 1), ylim = c(0, 1), pch = 20, col = "red",
     xlab = "PCR Prevalence", ylab = "Slide Prevalence")
lines(West_Africa_fitted_PCR, West_Africa_values/West_Africa_fitted_PCR, col = "red", lwd = 3)

West_Africa_data_values <- seq(0,1,0.001)
West_Africa_data_chains_sens <- West_Africa_micr_PCR_comp_model[[1]]
West_Africa_data_pred_mean_dist_sens <- matrix(NA, nrow = nrow(West_Africa_data_chains_sens), ncol = length(West_Africa_data_values))
for (i in 1:nrow(West_Africa_data_pred_mean_dist_sens)){
  West_Africa_data_value_logit_scale <- West_Africa_data_chains_sens[i, "delt"] + 
    (1 + West_Africa_data_chains_sens[i, "beta"]) * logit(West_Africa_data_values)
  West_Africa_data_pred_mean_dist_sens[i, ] <- expit(West_Africa_data_value_logit_scale)
}
West_Africa_data_credible_lower_sens <- apply(West_Africa_data_pred_mean_dist_sens, MARGIN = 2, quantile, prob = 0.025)
West_Africa_data_credible_upper_sens <- apply(West_Africa_data_pred_mean_dist_sens, MARGIN = 2, quantile, prob = 0.975)

# Actually = lower because doing the 1 -
West_Africa_data_sensitivity_upper <- West_Africa_data_values/West_Africa_data_credible_upper_sens

# Actually = upper because doing the 1 -
West_Africa_data_sensitivity_lower <- West_Africa_data_values/West_Africa_data_credible_lower_sens

polygon(y = c(West_Africa_data_values, rev(West_Africa_data_values)), 
        x = c(West_Africa_data_sensitivity_upper, West_Africa_data_sensitivity_lower), 
        col = adjustcolor("red", alpha.f = 0.5), border = NA)


#Plotting the sensitivities altogether
plot(0, 0, xlim = c(0, 1), ylim = c(0, 1), pch = 1, cex = 0.1,
     xlab = "PCR Prevalence", ylab = "Slide Prevalence")
lines(East_Africa_fitted_PCR, East_Africa_values/East_Africa_fitted_PCR, col = "green3", lwd = 3)
lines(West_Africa_fitted_PCR, West_Africa_values/West_Africa_fitted_PCR, col = "red", lwd = 3)
lines(Asia_fitted_PCR, Asia_values/Asia_fitted_PCR, col = adjustcolor("black"), lwd = 3)
lines(South_America_fitted_PCR, South_America_values/South_America_fitted_PCR, col = "blue", lwd = 3)


polygon(x = c(West_Africa_data_values, rev(West_Africa_data_values)), 
        y = c(West_Africa_data_sensitivity_upper, rev(West_Africa_data_sensitivity_lower)), 
        col = adjustcolor("red", alpha.f = 0.5), border = NA)
polygon(x = c(South_America_data_values, rev(South_America_data_values)), 
        y = c(South_America_data_sensitivity_upper, rev(South_America_data_sensitivity_lower)), 
        col = adjustcolor("blue", alpha.f = 0.5), border = NA)
polygon(x = c(East_Africa_data_values, rev(East_Africa_data_values)), 
        y = c(East_Africa_data_sensitivity_upper, rev(East_Africa_data_sensitivity_lower)), 
        col = adjustcolor("green3", alpha.f = 0.5), border = NA)
